...
  { key: "movie", content: {ua: "фільму", ru: "фильма", en:"movie"}, label: { ua: "Фільми", ru: "Фильмы", en: "Movie's" } },
  { key: "series", content: {ua: "серіалу", ru: "сериала", en:"series"}, label: { ua: "Серіали", ru: "Сериалы", en: "Series" } },
...
    { name: null, dislikes: [], likes: [], decade: 2000, isCharacter: false, content: null } 
  ],
...
        return { ...s, participants: [...s.participants, { name: null, dislikes: [], likes: [], decade: 2000, isCharacter: false, content: null }] };
      }
      return s;
    });
  };
...
            // БАГ 1: Сохраняем весь объект контента
...
         <PartnerChoice
            lang={state.lang}
            // Передаем объект контента для динамического перевода
            contentType= {state.participants[0].content} 
            value={state.partnerType}
            // onSelect: просто сохраняем ключ партнера
            onSelect={(selectedOption) => update({ partnerType: selectedOption.key })}
            onNext={() => {
...
              updateParticipant(1, { name, content: state.participants[0].content }); 
              nextStep();
            }}
          />
        )}
...
        {/* Summary */}
        {((state.step === 7) || (state.step === 6 && state.partnerType === "popular-character")) && (
          <Summary
